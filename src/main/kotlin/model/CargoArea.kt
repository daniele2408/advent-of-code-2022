...
class CargoArea(numberOfSlots : Int) {
    ...
    private fun parseMoveCommand(command: String) : Triple<Int, Int, Int> {
        val ls: List<Int> = command.split(" ").filter { it.all { c -> c.isDigit() } }.map { it.toInt() }.take(3)
        return Triple(ls[0], ls[1]-1, ls[2]-1)
    }

    fun executeCommands(rowsInput: List<String>, craneModel: CraneModel) {
        rowsInput.forEach { row ->
            val triple = parseMoveCommand(row)
            when (craneModel) {
                CraneModel.CRATE_MOVER_9000 -> moveNCrates(triple.first, triple.second, triple.third)
                CraneModel.CRATE_MOVER_9001 -> moveNCratesTogether(triple.first, triple.second, triple.third)
            }
        }
    }
